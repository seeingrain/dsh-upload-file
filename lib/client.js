...
`;
		...
		function ThumbImg({ src, alt, fallback }) {
			...
			if (failed) return typeof fallback === "function" ? fallback() : fallback ?? react.default.createElement(FileBadge, {
				name: alt,
				small: true
			});
		...
		const COPY_ICO = strokeSvg([react.default.createElement("rect", {
			x: 9,
			y: 9,
			width: 13,
			height: 13,
		...
		const DOWNLOAD_ICO = strokeSvg([
			...
			react.default.createElement("polyline", { points: "7 10 12 15 17 10" }),
		...
		function FileLibraryWindow({ queue, sessionId, inputActions, useInput, openFile }) {
			...
			const [newNames, setNewNames] = (0, react.useState)(() => /* @__PURE__ */ new Set());
			const knownRef = (0, react.useRef)(null);
			...
			const refresh = (0, react.useCallback)(async () => {
				try {
					...
					if (knownRef.current === null) knownRef.current = new Set(list.map((f) => f.name));
					else {
			...
			const iconFor = (entry) => {
				...
				if (family === "image" && !name.toLowerCase().endsWith(".svg")) return react.default.createElement(ThumbImg, {
					src: api.thumbUrl(sessionId, name),
					alt: name,
					fallback: () => react.default.createElement("img", {
						src: api.contentUrl(sessionId, name),
						alt: name,
						loading: "lazy",
						draggable: false
					})
				...
				if (family === "video" || family === "pdf") return react.default.createElement(ThumbImg, {
					src: api.thumbUrl(sessionId, name),
					alt: name
				});
